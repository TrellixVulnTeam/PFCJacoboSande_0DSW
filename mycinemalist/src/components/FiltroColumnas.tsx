...
export class FiltroColumnas extends React.Component<
  IFiltroColumnasProps,
  IFiltroColumnasState
> {
  public componentDidUpdate(
    prevProps: IFiltroColumnasProps,
    prevState: IFiltroColumnasState
  ) {
    if (
      prevProps.value != null &&
      prevProps.value != this.props.value &&
      prevProps.value != this.state.textoFiltro
    ) {
  ...
  public render(): JSX.Element {
    var MenuProps: IContextualMenuProps = {
      items: this.props.Opciones.map(
        (IFiltroColumnasOpcion: IFiltroColumnasOpcion): IContextualMenuItem => {
          ...
          if (IFiltroColumnasOpcion.SubOpcion) {
            var SubMenu = {
              items: IFiltroColumnasOpcion.SubOpcion.map(
                (
                  SubIFiltroColumnasOpcion: IFiltroColumnasOpcion
                ): IContextualMenuItem => {
                  let Properties = {
